...
ESX.RegisterServerCallback('mdt:server:assignVehicle', function(source, cb, payload)
  ...
  local model = payload.model
  local targetIdentifier = payload.targetIdentifier
  local targetName = payload.targetName or 'Client'
  local targetPlayerId = payload.targetPlayerId

  -- Get target player identifier if not provided
  if not targetIdentifier and targetPlayerId then
    local xTarget = ESX.GetPlayerFromId(targetPlayerId)
    if xTarget then
      targetIdentifier = xTarget.identifier
      targetName = GetPlayerName(targetPlayerId) or 'Client'
    end
  end
  ...
  MySQL.query('SELECT stock, name FROM vehicles WHERE model = ?', { model }, function(rows)
    ...
    MySQL.update('UPDATE vehicles SET stock = stock - 1 WHERE model = ? AND stock > 0', { model }, function(affectedRows)
      ...
      MySQL.insert([[
        ...
        refreshClients('dealership')
        sendWebhook('Vehicule attribue', {
          { name = 'Concessionnaire', value = playerLabel(source), inline = true },
          { name = 'Client', value = targetName, inline = true },
          { name = 'Vehicule', value = vehicleData.name or model, inline = true },
...
ESX.RegisterServerCallback('mdt:server:getProducts', function(source, cb)
  ...
  MySQL.query([[
    SELECT id, label, price, COALESCE(status, 'active') as status, description
    FROM mdt_products
    WHERE job_name = ?
    ORDER BY label ASC
  ]], { playerJob.name }, function(rows)
...
ESX.RegisterServerCallback('mdt:server:saveProduct', function(source, cb, payload)
  ...
  local playerJob = getPlayerJob(playerId)
  if not playerJob or not payload or not payload.label then
    cb({ ok = false, reason = 'invalid_payload' })
    return
  end
  ...
  if payload.id then
    ...
    MySQL.update([[
      UPDATE mdt_products
      SET label = ?, price = ?, status = ?, description = ?
      WHERE id = ? AND job_name = ?
    ]], {
      payload.label,
      payload.price or 0,
    ...
    -- Insert new product
    MySQL.insert([[
      INSERT INTO mdt_products (job_name, label, price, status, description)
      VALUES (?, ?, ?, ?, ?)
    ]], {
      playerJob.name,
      payload.label,
...
ESX.RegisterServerCallback('mdt:server:deleteProduct', function(source, cb, payload)
  ...
  local playerJob = getPlayerJob(playerId)
  if not playerJob or not payload or not payload.id then
    cb({ ok = false, reason = 'invalid_payload' })
    return
  end

  MySQL.update('DELETE FROM mdt_products WHERE id = ? AND job_name = ?', { payload.id, playerJob.name }, function()
    refreshClients('products')
    cb({ ok = true })
  end)
end)
...
-- ============================================
-- NEARBY PLAYERS (for invoices/dealership)
-- ============================================
...
ESX.RegisterServerCallback('mdt:server:getNearbyPlayers', function(source, cb)
  ...
  local nearbyPlayers = {}
  local maxDistance = 10.0 -- 10 meters
  ...
  -- Wait a bit for MySQL queries to complete
  Citizen.Wait(100)
...
ESX.RegisterServerCallback('mdt:server:getPlayerById', function(source, cb, payload)
  if not payload or not payload.playerId then
    cb({ ok = false, reason = 'invalid_payload' })
    return
  end

  local targetPlayer = ESX.GetPlayerFromId(payload.playerId)
...
-- ============================================
-- DOJ - ALL COMPANIES & TAX INVOICES
-- ============================================

ESX.RegisterServerCallback('mdt:server:getAllCompanies', function(source, cb)
  local playerJob = getPlayerJob(source)
  if not playerJob or playerJob.name ~= 'doj' then
    cb({ ok = false, reason = 'not_doj' })
    return
  end

  -- Get all jobs with society accounts
  MySQL.query([[
    SELECT
      j.name as job_name,
      j.label as job_label,
      COALESCE(a.money, 0) as society_balance,
      COALESCE(inv.total_invoiced, 0) as total_invoiced,
      COALESCE(inv.total_taxes, 0) as total_taxes,
      COALESCE(emp.employee_count, 0) as employee_count
    FROM jobs j
    LEFT JOIN addon_account_data a ON a.account_name = CONCAT('society_', j.name)
    LEFT JOIN (
      SELECT job_name, SUM(total_ttc) as total_invoiced, SUM(tax_amount) as total_taxes
      FROM mdt_invoices
      WHERE created_at >= DATE_SUB(NOW(), INTERVAL 30 DAY)
      GROUP BY job_name
    ) inv ON inv.job_name = j.name
    LEFT JOIN (
      SELECT job, COUNT(*) as employee_count
      FROM users
      GROUP BY job
    ) emp ON emp.job = j.name
    WHERE j.name NOT IN ('unemployed', 'doj')
    ORDER BY j.label
  ]], {}, function(rows)
    local totalTaxes = 0
    for _, row in ipairs(rows or {}) do
      totalTaxes = totalTaxes + (row.total_taxes or 0)
    end
    cb({ ok = true, companies = rows or {}, totalTaxes = totalTaxes })
  end)
end)

ESX.RegisterServerCallback('mdt:server:getTaxInvoices', function(source, cb)
  local playerJob = getPlayerJob(source)
  if not playerJob or playerJob.name ~= 'doj' then
    cb({ ok = false, reason = 'not_doj' })
    return
  ...
  MySQL.query([[
    SELECT
      ...
      i.created_at
    FROM mdt_invoices i
    LEFT JOIN jobs j ON j.name = i.job_name
...
ESX.RegisterServerCallback('mdt:server:getJobGrades', function(source, cb)
  local playerJob = getPlayerJob(source)
  if not playerJob then
    cb({ ok = false, reason = 'no_job' })
    return
  ...
  MySQL.query('SELECT grade, label FROM job_grades WHERE job_name = ? ORDER BY grade ASC', { playerJob.name }, function(rows)
    cb({ ok = true, grades = rows or {} })
  end)
end)

-- ============================================
-- EMPLOYEE MANAGEMENT BY IDENTIFIER
-- ============================================
...
ESX.RegisterServerCallback('mdt:server:hireEmployeeById', function(source, cb, payload)
  ...
  local bossJob = getPlayerJob(playerId)
  if not bossJob or not payload or not payload.targetId then
    cb({ ok = false, reason = 'invalid_payload' })
    return
  end
  ...
  target.setJob(bossJob.name, 0)
  refreshClients('employees')
  sendWebhook('Employe recrute', {
    { name = 'Patron', value = playerLabel(playerId), inline = true },
    { name = 'Employe', value = playerLabel(payload.targetId), inline = true },
    { name = 'Entreprise', value = bossJob.name, inline = true }
  })
  cb({ ok = true })
end)
...
ESX.RegisterServerCallback('mdt:server:fireEmployeeByIdentifier', function(source, cb, payload)
  ...
  MySQL.update('UPDATE users SET job = ?, job_grade = 0 WHERE identifier = ?', { Config.DefaultJob or 'unemployed', payload.identifier }, function()
    -- If player is online, update their job
    local players = ESX.GetExtendedPlayers()
    for _, xPlayer in pairs(players) do
      if xPlayer.identifier == payload.identifier then
        xPlayer.setJob(Config.DefaultJob or 'unemployed', 0)
        break
      end
    end

    refreshClients('employees')
    sendWebhook('Employe licencie', {
      { name = 'Patron', value = playerLabel(playerId), inline = true },
      { name = 'Employe', value = payload.identifier, inline = true }
    })
    cb({ ok = true })
  end)
end)
...
ESX.RegisterServerCallback('mdt:server:promoteEmployeeByIdentifier', function(source, cb, payload)
  ...
  local bossJob = getPlayerJob(playerId)
  if not bossJob or not payload or not payload.identifier or payload.newGrade == nil then
    cb({ ok = false, reason = 'invalid_payload' })
    return
  end

  -- Update user in database
  MySQL.update('UPDATE users SET job_grade = ? WHERE identifier = ? AND job = ?', { payload.newGrade, payload.identifier, bossJob.name }, function()
    -- If player is online, update their grade
    local players = ESX.GetExtendedPlayers()
    for _, xPlayer in pairs(players) do
      if xPlayer.identifier == payload.identifier then
        xPlayer.setJob(bossJob.name, payload.newGrade)
        break
      end
    end

    refreshClients('employees')
    sendWebhook('Employe promu', {
      { name = 'Patron', value = playerLabel(playerId), inline = true },
      { name = 'Employe', value = payload.identifier, inline = true },
      { name = 'Nouveau grade', value = tostring(payload.newGrade), inline = true }
    })
    cb({ ok = true })
  end)
end)
...
ESX.RegisterServerCallback('mdt:server:payCommission', function(source, cb, payload)
  local playerId = source
  if not isBoss(playerId) then
    cb({ ok = false, reason = 'no_permission' })
    return
  ...
  local playerJob = getPlayerJob(playerId)
  if not playerJob or not payload then
    cb({ ok = false, reason = 'invalid_payload' })
    return
  end

  local amount = tonumber(payload.amount) or 0
  local employeeIdentifier = payload.employeeIdentifier
  local employeeName = payload.employeeName or 'Employe'
  ...
  TriggerEvent('esx_addonaccount:getSharedAccount', 'society_' .. playerJob.name, function(account)
    local processPay = function(societyMoney)
      ...
      -- Find employee and pay them
      local targetPlayer = nil
      local players = ESX.GetExtendedPlayers()
      for _, xPlayer in pairs(players) do
        if xPlayer.identifier == employeeIdentifier then
      ...
      if targetPlayer then
        ...
        targetPlayer.addAccountMoney('bank', amount, 'Commission payout')
      else
        -- Player offline, add to database
        MySQL.update('UPDATE users SET bank = bank + ? WHERE identifier = ?', { amount, employeeIdentifier })
      end
      ...
      -- Update payout status if payoutId provided
      if payload.payoutId then
        MySQL.update('UPDATE mdt_commission_payouts SET status = ?, paid_at = NOW() WHERE id = ?', { 'paid', payload.payoutId })
      end

      -- Reset employee commission due
      MySQL.update('UPDATE mdt_employee_stats SET commission_due = 0 WHERE employee_identifier = ? AND job_name = ?', { employeeIdentifier, playerJob.name })

      refreshClients('commissions')
      refreshClients('employees')
      sendWebhook('Commission payee', {
        { name = 'Patron', value = playerLabel(playerId), inline = true },
        { name = 'Employe', value = employeeName, inline = true },
        { name = 'Montant', value = tostring(amount), inline = true }
      })
      cb({ ok = true })
    end
    ...
    if account then
      ...
      MySQL.query('SELECT money FROM addon_account_data WHERE account_name = ?', { 'society_' .. playerJob.name }, function(rows)
        local societyMoney = rows and rows[1] and rows[1].money or 0
        processPay(societyMoney)
      end)
    end
  end)
end)

ESX.RegisterServerCallback('mdt:server:payAllCommissions', function(source, cb, payload)
  local playerId = source
  if not isBoss(playerId) then
    cb({ ok = false, reason = 'no_permission' })
    return
  ...
  local playerJob = getPlayerJob(playerId)
  if not playerJob or not payload or not payload.payouts then
    cb({ ok = false, reason = 'invalid_payload' })
    return
  end

  local totalAmount = tonumber(payload.totalAmount) or 0
  local payouts = payload.payouts

  if totalAmount <= 0 or #payouts == 0 then
    cb({ ok = false, reason = 'nothing_to_pay' })
    return
  end
  ...
  TriggerEvent('esx_addonaccount:getSharedAccount', 'society_' .. playerJob.name, function(account)
    ...
    if account then
      ...
      local rows = MySQL.query.await('SELECT money FROM addon_account_data WHERE account_name = ?', { 'society_' .. playerJob.name })
      societyMoney = rows and rows[1] and rows[1].money or 0
    end

    if societyMoney < totalAmount then
      cb({ ok = false, reason = 'insufficient_funds' })
      return
    end

    -- Remove total from society
    if account then
      account.removeMoney(totalAmount)
    else
      MySQL.update('UPDATE addon_account_data SET money = money - ? WHERE account_name = ?', { totalAmount, 'society_' .. playerJob.name })
    end

    -- Pay each employee
    for _, payout in ipairs(payouts) do
      local amount = tonumber(payout.amount) or 0
      local employeeIdentifier = payout.employeeIdentifier

      if amount > 0 and employeeIdentifier then
        -- Find employee
        local targetPlayer = nil
        local players = ESX.GetExtendedPlayers()
        for _, xPlayer in pairs(players) do
        ...
        if payout.id then
          MySQL.update('UPDATE mdt_commission_payouts SET status = ?, paid_at = NOW() WHERE id = ?', { 'paid', payout.id })
        end

        -- Reset commission due
        MySQL.update('UPDATE mdt_employee_stats SET commission_due = 0 WHERE employee_identifier = ? AND job_name = ?', { employeeIdentifier, playerJob.name })
      end
    end
    ...
    refreshClients('commissions')
    refreshClients('employees')
    sendWebhook('Commissions payees en masse', {
      { name = 'Patron', value = playerLabel(playerId), inline = true },
      { name = 'Total', value = tostring(totalAmount), inline = true },
      { name = 'Employes', value = tostring(#payouts), inline = true }
    })
    cb({ ok = true })
  end)
end)
...
ESX.RegisterServerCallback('mdt:server:deletePartnership', function(source, cb, payload)
  ...
  MySQL.update('DELETE FROM mdt_partnerships WHERE id = ? AND job_name = ?', { payload.id, playerJob.name }, function()
    refreshClients('partnerships')
    cb({ ok = true })
  end)
end)

ESX.RegisterServerCallback('mdt:server:togglePartnershipStatus', function(source, cb, payload)
  local playerId = source
  if not isBoss(playerId) then
    cb({ ok = false, reason = 'no_permission' })
    return
  ...
  local playerJob = getPlayerJob(playerId)
  if not playerJob or not payload or not payload.id or not payload.status then
    cb({ ok = false, reason = 'invalid_payload' })
    return
  end

  MySQL.update('UPDATE mdt_partnerships SET status = ? WHERE id = ? AND job_name = ?', { payload.status, payload.id, playerJob.name }, function()
    refreshClients('partnerships')
    cb({ ok = true })
  end)
end)

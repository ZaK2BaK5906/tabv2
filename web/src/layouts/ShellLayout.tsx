...
const ShellLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const visibleRoutes = routes.filter((route) => {
    // DOJ/Taxes page - only visible for doj job
    if (route.path === '/taxes') {
      return player?.job?.name === 'doj';
    }
    // Dealership page - only visible for dealership/cardealer job
    if (route.path === '/dealership') {
      return player?.job?.name === 'dealership' || player?.job?.name === 'cardealer';
    }
    // All other routes are visible to everyone
    return true;
  });
